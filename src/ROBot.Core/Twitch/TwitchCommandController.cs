...
namespace ROBot.Core.Twitch
{
    public class TwitchCommandController : ITwitchCommandController
    {
        ...
        public async Task HandleAsync(IBotServer game, ITwitchCommandClient twitch, ChatCommand command)
        {
            ...
            if (handler == null)
            {
                logger.LogInformation("HandleAsync::Unknown Command: " + command.CommandIdentifier + ": " + command.CommandText);
                return;
            }

            await handler.HandleAsync(game, twitch, new TwitchCommand(command));
        }
